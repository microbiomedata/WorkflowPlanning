...
if [ "$run_checkm" == "true" ] && [[ ! -s work_files/binsM.stats ]]; then
	...
	for bin_set in $(ls | grep -v tmp | grep -v stats | grep bins); do 
		...
		if [ "$quick" == "true" ]; then
			...
			checkm lineage_wf -x fa $bin_set ${bin_set}.checkm -t $threads --pplacer_threads $p_threads --reduced_tree
		else
			checkm lineage_wf -x fa $bin_set ${bin_set}.checkm -t $threads --pplacer_threads $p_threads
...
if [ "$run_checkm" == "true" ] && [ $dereplicate != "false" ]; then
	...
	if [ "$quick" == "true" ]; then
		...
		checkm lineage_wf -x fa binsO binsO.checkm -t $threads --pplacer_threads $p_threads --reduced_tree
	else
		#checkm lineage_wf -x fa binsO binsO.checkm -t $threads --tmpdir binsO.tmp --pplacer_threads $p_threads
		checkm lineage_wf -x fa binsO binsO.checkm -t $threads --pplacer_threads $p_threads
	fi
