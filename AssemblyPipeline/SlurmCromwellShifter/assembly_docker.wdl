...
task run_assembly{
	...
	command {
		...
		#mkdir -p ${outdir}
		if [ ${pairedNumber} -eq 2 ]; then
			metawrap assembly -1 ${sep=" -2 " PairedReads} -l ${minLen} -o assembly -m ${mem}  ${"--" + assembler} -t ${cpu} 
		else
			# presume interleaved format
			mkdir -p assembly
			seqtk seq -1 ${PairedReads[0]} > assembly/read_1.fastq
			seqtk seq -2 ${PairedReads[0]} > assembly/read_2.fastq
			metawrap assembly -1 assembly/read_1.fastq -2 assembly/read_2.fastq -l ${minLen} -o assembly -m ${mem}  ${"--" + assembler} -t ${cpu}
		fi
